...
function getResults(query, zip) {
  ...
  var zipParse = zip.split('|');
  if (zipParse.length == 1){
  // URL to the api
  ...
  fetch(url)
    ...
    //grab lat and long
    const location = new google.maps.LatLng(zipParse[1], zipParse[2]);
    infowindow = new google.maps.InfoWindow();
    //userPosition
    const userPosition = { lat: parseFloat(zipParse[1]), lng: parseFloat(longitude) };
    //initalize the map
    const map = new google.maps.Map(document.getElementById("map"), {
      center: userPosition,
      zoom: 12,
